...
if(isset($_POST['submit'])) {
    ...
    // === VALIDASI TIPE DATA ===
    // cek apakah NIS melampaui batas (maksimal 10 karakter)
    if(!is_numeric($nis)) {
        $_SESSION['notif_error'] = "⚠️ NIS harus berisi angka! (format: numeric)";
    } elseif(strlen($nis) > 10) {
        $_SESSION['notif_error'] = "⚠️ NIS terlalu panjang! (maksimal: 10 karakter, saat ini: " . strlen($nis) . " karakter)";
    } 
    // cek apakah KELAS melampaui batas panjang (maksimal 10 karakter)
    elseif(strlen($kelas) > 10) {
        $_SESSION['notif_error'] = "⚠️ Kelas terlalu panjang! (maksimal: 10 karakter, saat ini: " . strlen($kelas) . " karakter)";
    }
    // jika validasi lulus, lanjutkan proses
    else {
        ...
        if(mysqli_num_rows($cek) > 0) {
            ...
            $query_insert = "INSERT INTO tb_siswa (nis, kelas) VALUES ('$nis', '$kelas')";
            
            if(mysqli_query($conn, $query_insert)) {
                // buat notifikasi sukses
                $_SESSION['notif_sukses'] = "data berhasil ditambahkan";
            } else {
                // ambil error code dari database
                $error = mysqli_error($conn);
                
                // cek jika error adalah duplicate entry untuk primary key
                if(strpos($error, 'Duplicate entry') !== false && strpos($error, 'PRIMARY') !== false) {
                    // error ini menunjukkan auto_increment sudah habis
                    $_SESSION['notif_error'] = "⚠️ SISTEM ERROR: Database primary key sudah mencapai batas maksimal!\nHubungi administrator untuk memperbaiki struktur database (ubah INT ke BIGINT pada kolom ID).";
                } else {
                    // error lainnya
                    $_SESSION['notif_error'] = "gagal menambahkan data. Error: " . $error;
                }
            }
        }
    }
